...
def save_audio(waveform: torch.Tensor, output_path: str, sample_rate: int = SAMPLE_RATE):
    """
    Low-pass filter at 7 kHz, quantize to signed 16-bit PCM, and save.
    """
    # 1) remove any ultrasonics above 7 kHz
    filtered = lowpass_biquad(waveform, sample_rate, cutoff_freq=7000)

    # 2) clamp to [-1,1], scale to int16 range, cast
    int16_waveform = (filtered.clamp(-1.0, 1.0) * 32767).to(torch.int16)

    # 3) write out as 16-bit PCM WAV
    torchaudio.save(
        output_path,
        int16_waveform,
        sample_rate,
...
watermarked_file = "inference_results/why_watermarked.wav"
detection_result = detect_watermark(watermarked_file, detector)
...
original_file = "Songs/Kanye West - Flashing Lights (Alternate Intro).mp3" 
detection_result = detect_watermark(original_file, detector)
print(f"\nFile: {original_file}")
print(f"Detection Probability: {detection_result['mean_probability']:.4f}")
print(f"Decision: {detection_result['decision']}")
